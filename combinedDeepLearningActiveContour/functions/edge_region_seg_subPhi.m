...
%------------------------------------------------------------------------
function [seg,phi,m_cnt] = edge_region_seg_subPhi(I,subI,m_cnt,init_mask,max_its,intEweight,edge_weight,DLweight,Dynamic_Window,display)

% load deep learning parameters
...
[Edge_fun_x, Edge_fun_y] = gradient(Edge_fun); 
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    u = sum(subI(upts))/(length(upts)+eps); % interior mean
    v = sum(subI(vpts))/(length(vpts)+eps); % exterior mean
    
    F = (subI(idx)-u).^2-(subI(idx)-v).^2;         % force from image information
    %dF_curvature = get_curvature(phi,idx);  % force from curvature penalty
    ...
    dphidt = F./max(abs(F)) + intEweight*dF_curvature+DLweight*dEdl+edge_weight*dF_edge;  % gradient descent to minimize energy
    ...
    if((display>0)&&(mod(its,20) == 0)) 
      showCurveAndPhi(subI,phi,its);  
    end
    
    if Dynamic_Window==1
     % resize phi
     Mroi=size(subI,1);
     mask_r=remap_mask(phi<=0,m_cnt,I);
     %[phi_r,mask_r]=resize_phi(phi,m_cnt,I);
    [subI,m_cnt]=mask2subImage(I,mask_r,Mroi);
    %m_cnt
